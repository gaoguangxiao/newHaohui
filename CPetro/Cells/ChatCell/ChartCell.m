...
-(void)setCellFrame:(ChartCellFrame *)cellFrame
{
    ...
    self.icon.image= [UIImage imageNamed:[chartMessage.messageType intValue] == 1?@"icon01":@"icon02"];
    ...
    self.chartView.timterShow.text  = chartMessage.createTime;
}
-(void)setBackGroundImageViewImage:(ChartContentView *)chartView from:(NSString *)from to:(NSString *)to
{
    UIImage *normal=nil ;
    if([chartView.chartMessage.messageType intValue]==1){
        normal = [UIImage imageNamed:from];
        normal = [normal stretchableImageWithLeftCapWidth:normal.size.width * 0.5 topCapHeight:normal.size.height * 0.7];
        chartView.userName.text= [CustomUtil getUserInfo].memberName != nil?[CustomUtil getUserInfo].memberName:@"";
        
    }else if([chartView.chartMessage.messageType intValue] == 2){
        
        normal = [UIImage imageNamed:to];
        normal = [normal stretchableImageWithLeftCapWidth:normal.size.width * 0.5 topCapHeight:normal.size.height * 0.7];
        chartView.userName.text= @"客服";
    }
